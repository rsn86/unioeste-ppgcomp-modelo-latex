\chapter{Resultados dos Comandos}
\label{cap_exemplos}
...
\begin{figure}[htb]
 ...
 \centering
  \begin{minipage}{0.4\textwidth}
    ...
    \includegraphics[scale=0.4]{Imagens/figure0.png}
  ...
  \begin{minipage}{0.4\textwidth}
    ...
    \includegraphics[scale=0.4]{Imagens/figure1.png}
    \legend{Fonte: \citeonline[p. 24]{araujo2012}}
  \end{minipage}
\end{figure}
...
\begin{alineas}
  ...
  \item no \abnTeX\ estão disponíveis os ambiente \texttt{incisos} e \texttt{subalineas}, que em suma são o mesmo que se criar outro nível de \texttt{alineas}, como nos exemplos à seguir:
...
Para as referências, preencha o arquivo \texttt{.bib} com suas referências. Complete cada referência com todos os campos disponíveis (no arquivo \texttt{.bib} que acompanha este modelo, foram disponibilizados os campos de cada uma das referências abaixo. Utilize como exemplo). Quanto mais completa sua referência, melhor.

\begin{itemize}
    \item Em artigos em revistas: \cite{ArtigoRevista}
    \item Livros: \cite{Livros}
    \item Capítulos em Livros: \cite{CapituloLivro}
    \item Artigo em de anais de eventos: \cite{ArtigoAnais}
    \item Manual de SW, procedimentos: \cite{Manual}
    ...
    \item Documentos exclusivamente disponíveis na Internet: \cite{Internet}
\end{itemize}
...
Normalmente não há problemas em usar caracteres acentuados em arquivos bibliográficos (\texttt{*.bib}). Porém, como as regras da ABNT fazem uso quase abusivo da conversão para letras maiúsculas, é preciso observar o modo como se escreve os nomes dos autores. Na ~\autoref{tabela-acentos} você encontra alguns
exemplos das conversões mais importantes. Preste atenção especial para `ç' e `í' que devem estar envoltos em chaves. A regra geral é sempre usar a acentuação neste modo quando houver conversão para letras maiúsculas.
...
\url{http://groups.google.com/group/abntex2} e faça sua contribuição à ferramenta.
